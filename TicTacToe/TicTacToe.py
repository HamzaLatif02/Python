...
def Player1(board):
    player1 = input("Player1 enter your position (row, column)(1 to 3): ")
    row = int(player1[0]) - 1
    column = int(player1[2]) - 1
    while board[row][column] == "X" or board[row][column] == "O":
        player1 = input("Invalid input. Player1 enter your position (row, column)(1 to 3): ")
        row = int(player1[0]) - 1
        column = int(player1[2]) - 1
    board[row][column] = "X"
    PrintBoard(board)
...
def Player2(board):
    player2 = input("Player2 enter your position (row, column)(1 to 3): ")
    row = int(player2[0]) - 1
    column = int(player2[2]) - 1
    while board[row][column] == "X" or board[row][column] == "O":
        player2 = input("Invalid input. Player2 enter your position (row, column)(1 to 3): ")
        row = int(player2[0]) - 1
        column = int(player2[2]) - 1
    board[row][column] = "O"
    PrintBoard(board)
...
def ComputerPlayer(board):
    row = randint(0,2)
    column = randint(0,2)
    print("Computer enter your position (row,column)(1 to 3): " + str(row) + "," + str(column))
    while board[row][column] == "X" or board[row][column] == "O":
        row = randint(0,2)
        column = randint(0,2)
        print("Invalid position. Computer enter your position (row,column)(1 to 3): " + str(row) + "," + str(column))
    board[row][column] = "O"
    PrintBoard(board)
...
def PlayGame(c, PlayCount, Opponent):
    while c == 1:
        if Opponent == "Player2" or Opponent == "player2":
            Player1(board)
            PlayCount += 1
